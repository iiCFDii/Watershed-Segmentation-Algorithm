function output = ThresholdDoubleWater(image,low,high)    % Threshold Function
    [height,width,depth] = size(image);
    if depth > 1
        image = im2gray(image);
    end
    output = zeros(height,width);    
    parts = zeros(height,width); % This is used to collect all the seperate floodfill outputs
    image_tlow = ThresholdSingle(image,low);    % Low threshold image
    image_thigh = ThresholdSingle(image,high);   % High threshold image
        ...
        for j=1:width 
            ...
            % the picture we want will get flood filled
             if image_thigh(i,j) == 254 && image_tlow(i,j) ~= 255 
                 ...
                 parts = double(parts)+double(output);
